...
- **Planes**: Do smaller planes (measured by the number of seats) have less departure delay? NO!
- **Airlines I**: 
  - **Flight distance**: Calculate the average flight distance by airline and display the top 10 airlines
  - **Delay**: What's the average delay by airline? (But note that this will be strongly correlated with flight distance, so you may want to account for that) Done ! 
  
  Could Join the names of airlines
...
df_inner %>%
  group_by(model) %>%
  summarise( avgdepdelay = mean(dep_delay), seats= mean(seats)) %>%
  arrange(desc(avgdepdelay))

df_inner %>%
  group_by(carrier) %>%
  summarise(avgflightdist = mean(distance), avgdelay = mean(arr_delay, na.rm = TRUE)) %>%
  arrange(desc(avgflightdist))
